...
public class OrderItemsProductsJoin {
    ...
    public static void main(String[] args) {
        ...
        props.put(StreamsConfig.APPLICATION_ID_CONFIG, "pizzashop-orderitems7");
        ...
        props.put(StreamsConfig.DEFAULT_VALUE_SERDE_CLASS_CONFIG, OrderItemSerde.class.getName());
...
//        KStream<Bytes, OrderItem> orderItems = orders.flatMapValues((ValueMapper<Order, Iterable<OrderItem>>) order -> order.items);



        KStream<Bytes, OrderItem> orderItems = orders.flatMap((KeyValueMapper<Bytes, Order, Iterable<KeyValue<Bytes, OrderItem>>>) (bytes, order) -> {
            List<KeyValue<Bytes, OrderItem>> result = new ArrayList<>();

            for (OrderItem item : order.items) {
                result.add(new KeyValue(new Bytes(String.valueOf(item.productId).getBytes()), item));
            }
            return result;
        });
        ...
        KStream<Bytes, HydratedOrderItem> enrichedOrderItems = orderItems.leftJoin(products, (orderItem, product) -> {
            ...
            hydratedOrderItem.orderItem = orderItem;
